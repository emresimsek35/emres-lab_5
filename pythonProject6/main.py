...
def rep_with_upper(user_string):
    chars = list(user_string)
    num = random.randint(1, len(user_string))
    changed_string = ''

    for i in range(0, int(len(chars))):
        if i == num:
            chars[num] = chars[num].upper()
            changed_string = ''.join(chars)

    return changed_string


def swap_letters(user_string):
    chars = list(user_string)
    a = chars[0]
    b = chars[1]
    chars[0] = chars[-2]
    ...
    chars[-2] = a
    chars[-1] = b
    changedString = ''.join(chars)
    return changedString


def search_letter(user_string, keyLetter):
    if user_string.find(keyLetter) != -1:
        return True
    else:
        return False


userNumber = input("Please enter a number between 3 and 7: ")
x = True
password = ''
generatedPass = ""
while x:
    if int(userNumber) < 3 or int(userNumber) > 7:
        print("Invalid value please try again!")
        x = False
    else:
        generatedPass = passGen(userNumber)
        print(generatedPass)
    x = False

upperPass = rep_with_upper(generatedPass)
print(upperPass)
swapPass = swap_letters(upperPass)
print(swapPass)
print("initial password: " + generatedPass)
print("final password: " + swapPass)
print(search_letter(swapPass, 'a'))

letter = input("Please enter the letter to be searched in password: ")
print(search_letter(swapPass, letter))
